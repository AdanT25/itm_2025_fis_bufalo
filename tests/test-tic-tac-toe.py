...
class TestBoard:
    ...
    def test_board_make_move(self):
        ...
        board = Board()
        assert board.make_move(0, 'X') == True
    ...
    def test_board_invalid_move(self):
        ...
        board = Board()
        board.make_move(0, 'X')
        assert board.make_move(0, 'O') == False
    
    def test_board_out_of_range(self):
        """Test 5: No debe permitir movimiento fuera de rango."""
        board = Board()
        assert board.make_move(9, 'X') == False
        assert board.make_move(-1, 'X') == False
...
class TestIntegration:
    ...
    def test_complete_game_flow(self):
        ...
        # Jugador X en centro
        assert game.process_move(4) == True
        assert game.board.cells[4] == 'X'
        
        # Jugador O en esquina
        game.switch_player()
        assert game.process_move(0) == True
