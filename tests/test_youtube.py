...
def test_download_audio(mock_ffmpeg, mock_ydl):
    ...
    runner = CliRunner()
    result = runner.invoke(youtube, ["download", "https://youtube.com/watch?v=123", "--type", "audio"])
...
def test_download_video(mock_ffmpeg, mock_ydl):
    ...
    assert any(pp["key"] == "FFmpegVideoConvertor" for pp in call_args["postprocessors"])
